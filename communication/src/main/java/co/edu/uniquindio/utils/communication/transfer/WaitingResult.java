...
class WaitingResult<T> extends Thread {
	 ...
	 * @param sequence
	 *            . The sequence number of the message
	 * @param returnsManagerCommunication
	 */
	...
	public void run() {
		...
		if (!stop) {
			...
			logger
					.finest("Timeout waiting for a response for number sequence= '"
							+ sequence + "'");

			logger.fine("Timeout waiting for a response");
	 ...
	 * 
	 * @param result
	 *            . This is the response of the message
	 */
